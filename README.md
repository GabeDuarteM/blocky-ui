...
- Search through query logs and filter them (requires [query logging](https://0xerr0r.github.io/blocky/latest/configuration/#query-logging) configured on Blocky)
  ...
  - CSV history is read from retained daily files automatically; unchanged file summaries are cached in memory. The first request for a large history or a new filter can take longer.
- Statistics sections
