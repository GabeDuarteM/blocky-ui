...
if (!process.env["DOCKER_HOST"]) {
  ...
  if (!isSocketAccessible(dockerSocket)) {
    // Check Colima socket (macOS Lima-based Docker runtime)
    ...
    if (isSocketAccessible(colimaSocket)) {
      ...
      // Ryuk (the testcontainers reaper) bind-mounts the Docker socket into its
      // own container using the path from DOCKER_HOST. The Colima socket lives
      // at a non-standard host path and cannot be resolved from inside a
      // container, so Ryuk fails to start. Disable it; containers will be
      // cleaned up by the afterAll hook instead.
